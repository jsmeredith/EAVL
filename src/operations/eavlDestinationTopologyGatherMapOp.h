...
#ifndef EAVL_DESTINATION_TOPOLOGY_GATHER_MAP_OP_H
#define EAVL_DESTINATION_TOPOLOGY_GATHER_MAP_OP_H
...
template <class CONN>
struct eavlDestinationTopologyGatherMapOp_CPU
{
...
__global__ void
eavlDestinationTopologyGatherMapOp_kernel(int nitems, CONN conn,
                                   const IN inputs, OUT outputs,
                                   INDEX indices, F functor)
{
    int *sparseindices = get<0>(indices).array;
...
template <class CONN>
struct eavlDestinationTopologyGatherMapOp_GPU
{
                     ...
                     INDEX indices, F &functor)
    {
        ...
        dim3 blocks (32,           1, 1);
        eavlDestinationTopologyGatherMapOp_kernel<<< blocks, threads >>>(nitems, conn,
                                                                  inputs, outputs,
                                                                  indices, functor);
        CUDA_CHECK_ERROR();
    }
...
// ****************************************************************************
// Class:  eavlDestinationTopologyGatherMapOp
//
// Purpose:
...
class eavlDestinationTopologyGatherMapOp : public eavlOperation
{
  protected:
    eavlCellSet *cells;
    ...
    F            functor;
  public:
    eavlDestinationTopologyGatherMapOp(eavlCellSet *c, eavlTopology t,
    ...
    virtual void GoCPU()
    {
        ...
        if (elExp)
        {
            ...
            eavlOpDispatch<eavlDestinationTopologyGatherMapOp_CPU<eavlExplicitConnectivity> >(n, conn, inputs, outputs, indices, functor);
        }
        else if (elStr)
        {
            eavlRegularConnectivity conn = eavlRegularConnectivity(elStr->GetRegularStructure(),topology);
            eavlOpDispatch<eavlDestinationTopologyGatherMapOp_CPU<eavlRegularConnectivity> >(n, conn, inputs, outputs, indices, functor);
        }
    }
    virtual void GoGPU()
    {
        ...
        if (elExp)
        {
            ...
            eavlOpDispatch<eavlDestinationTopologyGatherMapOp_GPU<eavlExplicitConnectivity> >(n, conn, inputs, outputs, indices, functor);
        ...
        {
            eavlRegularConnectivity conn = eavlRegularConnectivity(elStr->GetRegularStructure(),topology);
            eavlOpDispatch<eavlDestinationTopologyGatherMapOp_GPU<eavlRegularConnectivity> >(n, conn, inputs, outputs, indices, functor);
        }
#else
...
template <class I, class O, class INDEX, class F>
eavlDestinationTopologyGatherMapOp<I,O,INDEX,F> *new_eavlDestinationTopologyGatherMapOp(eavlCellSet *c, eavlTopology t,
                                                                   I i, O o, INDEX indices, F f) 
{
    return new eavlDestinationTopologyGatherMapOp<I,O,INDEX,F>(c,t,i,o,indices,f);
}
