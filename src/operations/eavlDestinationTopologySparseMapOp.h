...
template <class CONN>
struct eavlDestinationTopologySparseMapOp_CPU
{
...
__global__ void
eavlDestinationTopologySparseMapOp_kernel(int nitems, CONN conn,
                                   const IN inputs, OUT outputs,
                                   INDEX indices, F functor)
{
    int *sparseindices = get<0>(indices).array;
...
template <class CONN>
struct eavlDestinationTopologySparseMapOp_GPU
{
                     ...
                     INDEX indices, F &functor)
    {
        ...
        dim3 blocks (32,           1, 1);
        eavlDestinationTopologySparseMapOp_kernel<<< blocks, threads >>>(nitems, conn,
                                                                  inputs, outputs,
                                                                  indices, functor);
        CUDA_CHECK_ERROR();
    }
...
// ****************************************************************************
// Class:  eavlDestinationTopologySparseMapOp
//
// Purpose:
...
class eavlDestinationTopologySparseMapOp : public eavlOperation
{
  protected:
    eavlCellSet *cells;
    ...
    F            functor;
  public:
    eavlDestinationTopologySparseMapOp(eavlCellSet *c, eavlTopology t,
    ...
    virtual void GoCPU()
    {
        ...
        if (elExp)
        {
            ...
            eavlOpDispatch<eavlDestinationTopologySparseMapOp_CPU<eavlExplicitConnectivity> >(n, conn, inputs, outputs, indices, functor);
        }
        else if (elStr)
        {
            eavlRegularConnectivity conn = eavlRegularConnectivity(elStr->GetRegularStructure(),topology);
            eavlOpDispatch<eavlDestinationTopologySparseMapOp_CPU<eavlRegularConnectivity> >(n, conn, inputs, outputs, indices, functor);
        }
    }
    virtual void GoGPU()
    {
        ...
        if (elExp)
        {
            ...
            eavlOpDispatch<eavlDestinationTopologySparseMapOp_GPU<eavlExplicitConnectivity> >(n, conn, inputs, outputs, indices, functor);
        ...
        {
            eavlRegularConnectivity conn = eavlRegularConnectivity(elStr->GetRegularStructure(),topology);
            eavlOpDispatch<eavlDestinationTopologySparseMapOp_GPU<eavlRegularConnectivity> >(n, conn, inputs, outputs, indices, functor);
        }
#else
...
template <class I, class O, class INDEX, class F>
eavlDestinationTopologySparseMapOp<I,O,INDEX,F> *new_eavlDestinationTopologySparseMapOp(eavlCellSet *c, eavlTopology t,
                                                                   I i, O o, INDEX indices, F f) 
{
    return new eavlDestinationTopologySparseMapOp<I,O,INDEX,F>(c,t,i,o,indices,f);
}
