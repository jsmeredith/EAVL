...
void eavlRayTracerMutator::traversalTestISPC(int warmupRounds, int testRounds)
{
#ifdef HAVE_ISPC
    Init();
    eavlIntArray    *dummy= new eavlIntArray("",1,size);
    ...
    {
        /*int threads = 8;
        float ftasksSize = size / (float)threads;
        int taskSize = (int)ceil(ftasksSize);

        for(int p=0; p<8;p++)
        {
            int start = p * taskSize;
            int end = (p + 1) * taskSize ;
            if(p == threads - 1) end = min(end, size);
            cout<<"Thread "<<p<<" First "<<start<<" last "<<end<<endl;
        }*/ 

#ifdef HAVE_OPENMP 
        int threads = omp_get_num_threads();
        int tasks = 16;
        float ftasksSize = size / (float)tasks;
        int taskSize = (int)ceil(ftasksSize);
        //if(size % threads > 0) tasks++;
        //cout<<"Max Threads "<<omp_get_max_threads()<<" num threads "<<omp_get_num_threads()<<endl;
        #pragma omp parallel for 
        for (int index = 0; index < tasks; ++index)
        {   //cout<<"Thread "<<index<<" "<<omp_get_thread_num();
            int threadIdx = omp_get_thread_num();
            int start = index * taskSize;
            int end = (index + 1) * taskSize ;
            if(index == tasks - 1) end = min(end, size);
            ispc::intersectTriHelper((float*) rayDirX->GetHostArray(), (float*) rayDirY->GetHostArray(), (float*) rayDirZ->GetHostArray(),
                     (float*) rayOriginX->GetHostArray(), (float*) rayOriginY->GetHostArray(), (float*) rayOriginZ->GetHostArray(),
                     tri_bvh_in_raw, tri_bvh_lf_raw,
                     (float*) hitIdx->GetHostArray(), (float*) dummyFloat->GetHostArray(),
                     tri_verts_raw, start, end);
        }
